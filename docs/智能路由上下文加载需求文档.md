...
1. 一个 `session_id` 对应且只对应一个用户。
2. 不允许多个用户共用同一个 `session_id`。
3. session 默认空闲 30 分钟过期。
4. 空闲时间从该 session 最近一次有效请求或状态更新开始计算。
5. session 过期后，必须清除该 session 下的全部内存状态，包括 `slot_memory`、`task_list`、`current_task`、`context_leases` 和轻量 graph/task 状态。
6. session 过期后再次收到相同 `session_id` 的请求，应按新 session 处理，不继承历史任务、槽位或上下文 lease。
7. session 过期清理不应影响其他 session。
...
3. session 过期检查。
4. `agent.md` 加载。
5. skill metadata 索引生成。
6. 单意图或多意图识别结果。
7. task 创建或更新。
8. skill body 加载。
9. reference 可用列表。
10. reference body 加载或拒绝。
11. prompt 构建。
12. LLM 原始输出。
13. planner 分析结果。
14. 助手协议帧生成。
...
1. 默认空闲过期时间为 30 分钟。
2. 每次有效请求或状态更新后刷新 `last_active_at` 和 `expires_at`。
3. 请求进入时必须先检查 session 是否过期。
4. 已过期 session 必须先清理内存状态，再按新 session 初始化。
5. session 清理必须同时释放全部 task、slot memory、context lease 和轻量 graph/task 状态。
6. `session_id` 必须绑定单个用户；如果发现同一 `session_id` 被不同用户使用，应拒绝请求或重新分配 session，不得合并状态。
...
3. 检查 session 用户绑定关系
4. 检查 session 是否空闲过期
5. 如果 session 已过期，清理该 session 全部内存状态并重新初始化
6. 加载 `agent.md`
7. 判断是否存在 active task
8. 如果输入归属 active task，基于 lease 重载上下文
9. 如果是新任务或切换任务，生成 skill metadata index
10. 执行单意图或多意图识别
11. 校验 intent 结果
12. 创建或更新 task
13. 加载 task 对应 skill body
14. 暴露允许 reference 列表
15. planner 请求 reference
16. 校验并加载 reference body
17. planner 完成提槽、澄清或 handoff 判断
18. 生成助手协议帧
19. 保存 session 状态和 lease
20. 刷新 session `last_active_at` 和 `expires_at`
21. 释放已结束 task 的 lease
...
1. `request_received`
2. `session_loaded`
3. `session_user_bound`
4. `session_expiration_checked`
5. `session_expired`
6. `session_memory_cleared`
7. `agent_context_loaded`
8. `spec_progressive_load`
9. `intent_metadata_selected`
10. `multi_intent_detected`
11. `task_created`
12. `task_updated`
13. `skill_body_loaded`
14. `references_available`
15. `reference_body_loaded`
16. `reference_load_denied`
17. `prompt_loaded`
18. `llm_raw_response`
19. `llm_analysis`
20. `slot_and_skill_result`
21. `assistant_protocol_frames`
22. `context_released`
...
11. `session_id` 必须绑定单个用户，禁止跨用户复用。
12. session 过期清理必须只作用于当前过期 session，不影响其他用户 session。
...
14. 一个 `session_id` 只能绑定一个用户。
15. session 空闲 30 分钟后过期。
16. session 过期后清除 `slot_memory`、`task_list`、`current_task`、`context_leases` 和轻量 graph/task 状态。
17. 过期后使用相同 `session_id` 再次请求时，应按新 session 处理。
18. 一个 session 过期清理不影响其他 session。
